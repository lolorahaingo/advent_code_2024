...
point_s *find_pairs_n_add_antinodes(point_s *points, size_t count, size_t *antinodes_count, int width, int height) {
    size_t k = 0;
    int diag = sqrt(width*width + height*height);
    printf("diag = %d\n", diag);
    size_t capacity = diag;
    point_s *antinodes = malloc(capacity * sizeof(point_s));
    for (size_t i = 0; i < count; i++) {
        for (size_t j = i + 1; j < count; j++) {
            if (points[i].type == points[j].type) {
                // Calcul des vecteurs antinœuds
                float x1 = (points[i].x - points[j].x);
                float y1 = (points[i].y - points[j].y);
                float x2 = (points[j].x - points[i].x);
                float y2 = (points[j].y - points[i].y);
                for (int i2 = 0; i2<diag; i2++) {
                    // if (points[i].x + i2*x1 > width || points[i].y + i2*y1 > height 
                    // || points[i].x + i2*x1 < 0 || points[i].y + i2*y1 > height) break;
                    // Vérifie si l'antinœud existe déjà avant de l'ajouter
                    if (!antinode_exists(antinodes, k, points[i].x + i2*x1, points[i].y + i2*y1)) {
                        if (k + 1 > capacity) {
                            capacity *= 2;
                            antinodes = realloc(antinodes, capacity * sizeof(point_s));
                        }
                        antinodes[k].x = points[i].x + i2*x1;
                        antinodes[k].y = points[i].y + i2*y1;
                        antinodes[k++].type = '#';
                    }
                }
                for (int i2 = 0; i2<diag; i2++) {
                    // if (points[i].x + i2*x2 > width || points[i].y + i2*y2 > height 
                    // || points[i].x + i2*x2 < 0 || points[i].y + i2*y2 > height) break;
                    // Vérifie si l'antinœud existe déjà avant de l'ajouter
                    if (!antinode_exists(antinodes, k, points[j].x + i2*x2, points[j].y + i2*y2)) {
                        if (k + 1 > capacity) {
                            capacity *= 2;
                            antinodes = realloc(antinodes, capacity * sizeof(point_s));
                        }
                        antinodes[k].x = points[j].x + i2*x2;
                        antinodes[k].y = points[j].y + i2*y2;
                        antinodes[k++].type = '#';
                    }
                }
            }
...
int main(int argc, char *argv[]) {
    ...
    point_s *antinodes = find_pairs_n_add_antinodes(points, nb_antenna, &antinodes_count, width, height);
