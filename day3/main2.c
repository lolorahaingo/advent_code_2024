...
int process_memory(FILE *file, char enable) {
    int total_sum = 0;
    char ch;
    int x, y;

    while ((ch = fgetc(file)) != EOF) { // Lire caractère par caractère
        if (ch == 'm' && enable) { // Si on trouve "mul("
            // Vérifier "mul("
        ...
        else if (ch == 'd' && fgetc(file) == 'o') {
            char first = fgetc(file);
            char second = fgetc(file);
            if (first == '(' && second == ')')
                total_sum += process_memory(file, 1);
            else if (first == 'n' && second == '\'' && fgetc(file) == 't' && fgetc(file) == '(' && fgetc(file) == ')')
                total_sum += process_memory(file, 0);
...
int main(int argc, char *argv[]) {
    ...
    int result = process_memory(file, 1);
